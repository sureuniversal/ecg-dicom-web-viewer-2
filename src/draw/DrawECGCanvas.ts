    ...
    private drawECG() {
      ...
      for(let ileads = 0; ileads < this.dataMg.leads.length; ileads++){
        ...
        while (i < data.length && time < space) {
          //10mV/s:
          let point = data[i] * 600 * this.configuration.AMPLITUDE; /*((data[i] * objPosition.height) * this.configuration.AMPLITUDE); */  //Rescalate. 10mV/s Each square is 1 mm.
          //Draw line:
          this.ctx.beginPath();
          this.drawLine(
            startX + time,
          ...
          time += space / (5800 * (space / 910)) * (this.configuration.TIME / 0.25); /* (this.dataMg.duration / this.configuration.TIME) / 100; */
          i++;
        }
      };
      //Clear data:
